...
const Card = () => {
  const { gridView, setGridView, darkMode, setDarkMode } =
  ...
  const productNames =
    filteredProducts?.map((product) => product.productName) || []

  const productsPerPage = 8
  const [visibleProducts, setVisibleProducts] = useState(
    currentProducts.slice(0, productsPerPage)
  )

  const loadMoreProducts = () => {
    const remainingProducts = currentProducts.slice(visibleProducts.length)
  ...
  if (
    ...
    currentProducts.filter((product) => product.productName === searchTerm)
      .length == 1
  ) {
    currentProducts = currentProducts.filter(
      (product) => product.productName == filterNames[0]
    )
    ...
    <>
      <div className={`card_container ${darkMode ? 'dark-mode' : ''}`}>
        ...
        <section>
          <FreeHit />
          <div>
            <h1>Free-Hit's Tools!</h1>
          </div>
        </section>
        <div className="card_view">
          {totalPages > 1 && searchTerm.length == 0 && (
            <Pagination totalPages={totalPages} atTop />
          )}
